...
def create_app():
    ...
    # Initialize response compression for better performance when installed.
    if COMPRESS_AVAILABLE:
        compress = Compress()
        compress.init_app(app)
        logger.info("[OK] Response compression enabled")
    else:
        logger.warning("[WARNING] Response compression disabled")
    ...
    try:
        ...
        if app.config.get('DEBUG') or os.environ.get('AUTO_CREATE_DB', 'true').lower() == 'true':
