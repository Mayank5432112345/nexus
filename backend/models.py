...
class Vector(TypeDecorator):
    ...
    PostgreSQL uses pgvector. SQLite stores vectors as JSON text so local
    development works without a database extension.
    """
    impl = Text
    cache_ok = True

    def __init__(self, dimensions=384, *args, **kwargs):
...
def ensure_provider_columns():
    """Ensure provider_name and provider_user_id columns exist on users table."""
    try:
        from sqlalchemy import inspect, text
        inspector = inspect(db.engine)
        columns = [col['name'] for col in inspector.get_columns('users')]
        ...
        if 'provider_name' not in columns:
            if dialect_name == 'postgresql':
                ...
                stmts.append("ALTER TABLE users ADD COLUMN provider_name VARCHAR(50);")
        if 'provider_user_id' not in columns:
            if dialect_name == 'postgresql':
                stmts.append("ALTER TABLE users ADD COLUMN IF NOT EXISTS provider_user_id VARCHAR(200);")
            else:
                stmts.append("ALTER TABLE users ADD COLUMN provider_user_id VARCHAR(200);")
